...
class Board:
    ...
    def get_shortest_path(self, player):
        ...
        while len(neighbors) > 0:
            ...
            if x == self.goals[player]:
                ...
                #print("get shortest play:" + str(player) + ':' +
                  #  str(end - start) + 'succ: ' + str(succ))
                return succ
    ...
    def get_legal_wall_moves_colsely_filtered(self, player):
...
# walls for 0
        if player == 0:
            #horizontal walls for 0
                myRange = (self.pawns[1][1] - 1, self.pawns[1][1])
                if myRange[0] < 0:
                        myRange = (0, myRange[1])
                if myRange[1] > self.size - 1:
                        myRange = (myRange[0] , self.size - 1)

                i = myRange[0]
                while( i <= myRange[1]):

                        #append H walls directly in front of opponent
                        if(self.pawns[1][0] > 0 and self.is_wall_possible_here((self.pawns[1][0] - 1, i), True)):
                                moves.append(('WH', self.pawns[1][0] - 1, i))
                        i += 1

                #vertical for player 0
                myRange = (self.pawns[1][0] - 1, self.pawns[1][0])
                ...
                while( i <= myRange[1]):
                        ...
                        #append V walls directly to the LEFT of the opponent
                        if(self.pawns[1][1] > 0 and self.is_wall_possible_here((i, self.pawns[1][1] - 1), False)):
                                moves.append(('WV', i, self.pawns[1][1] - 1))

                        #append V walls directly to the RIGHT of the opponent
                        if(self.pawns[1][1] < self.size - 1 and self.is_wall_possible_here((i, self.pawns[1][1]), False)):
                                moves.append(('WV', i, self.pawns[1][1]))
                        i += 1

        # walls for 1
        if player == 1:

                #horizontal walls for 1
                myRange = (self.pawns[0][1] - 1, self.pawns[0][1])

                if myRange[0] < 0:
                        myRange = (0, myRange[1])
                if myRange[1] > self.size - 2:
                        myRange = (myRange[0] , self.size - 2)

                i = myRange[0]
                while( i <= myRange[1]):

                        #append H walls directly in front of opponent
                        if(self.pawns[0][0] < self.size - 1 and self.is_wall_possible_here((self.pawns[0][0] , i), True)):
                                moves.append(('WH', self.pawns[0][0], i))
                        i += 1

                #vertical for player 1
                myRange = (self.pawns[0][0] - 1, self.pawns[0][0])
                ...
                while( i <= myRange[1]):
                        ...
                        #append V walls directly to the LEFT of the opponent
                        if(self.pawns[0][1] > 0 and self.is_wall_possible_here((i, self.pawns[0][1] - 1), False)):
                                moves.append(('WV', i, self.pawns[0][1] - 1))

                        #append V walls directly to the RIGHT of the opponent
                        if(self.pawns[0][1] < self.size - 1 and self.is_wall_possible_here((i, self.pawns[0][1]), False)):
                                moves.append(('WV', i, self.pawns[0][1]))
                        i += 1
