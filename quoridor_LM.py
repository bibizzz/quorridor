...
class Board:
    ...
    def __init__(self, percepts=None):
        ...
        self.pawns = [(0, mid), (dim-1, mid)]
        self.goals = [dim-1, 0]
        self.nb_walls = [self.starting_walls, self.starting_walls]
        self.horiz_walls = []
        self.verti_walls = []
        self.shortest_path =[[], []]
        self.shortest_path_update = [False,  False]
...
#            print ("path clone" + str(path))
#            print (i)
            for (x, y) in path:
                clone_board.shortest_path[i].append((x, y))
            i += 1
        clone_board.shortest_path_update[0] = self.shortest_path_update[0]
        clone_board.shortest_path_update[1] = self.shortest_path_update[1]
        return clone_board
    ...
    def paths_exist(self):
        ...
        """
        try:
            p0 = self.min_steps_before_victory(0)
            p1 = self.min_steps_before_victory(1)
   #         print ("path exists:" + str(p0) + ' ' + str(p1))
            return True
        except NoPath:
            return False
    ...
    def get_shortest_path(self, player):
        ...
        if self.shortest_path_update[player] == True:
        ...
        while len(neighbors) > 0:
            ...
            if x == self.goals[player]:
                ...
                while curr is not None and curr != self.pawns[player]:
                    ...
                    curr = prede[x_][y_]
                succ.reverse()
                self.shortest_path[player]= succ
                self.shortest_path_update[player] = True
                #print ("Shortest path:" + str(succ))
                return succ
    ...
    def is_simplified_wall_possible_here(self, pos, is_horiz):
        ...
        with direction specified by is_horiz without considering if there is still a path.
        Aim : avoid multiple call to get_shortest_path with the same state.
        """
        self.shortest_path_update = [False,  False]
        (x, y) = pos
    ...
    def is_wall_possible_here(self, pos, is_horiz):
        ...
        (x, y) = pos
        self.shortest_path_update = [False,  False]
        #print("is wall pos" + str(x) + ' ' + str(y) + '' + str(is_horiz))
        if x >= self.size - 1 or x < 0 or y >= self.size - 1 or y < 0:
            return False
        #add the horizontal wall that makes a cross with the vertical wall (or vice versa)
        if not (tuple(pos) in self.horiz_walls or
                tuple(pos) in self.verti_walls):
            wall_horiz_right = (x, y + 1) in self.horiz_walls
            wall_horiz_left = (x, y - 1) in self.horiz_walls
            ...
            wall_vert_down = (x + 1, y) in self.verti_walls
            if is_horiz:
 #               cross = (x + 1,  y + 1) in self.verti_walls
                if wall_horiz_right or wall_horiz_left:
                    return False
   ...
   #add the horizontal wall that makes a cross with the vertical wall (or vice versa)
#                cross = (x - 1,  y - 1) in self.horiz_walls
                if wall_vert_up or wall_vert_down:
                    return False
    ...
    def is_path_valid(self,  path, player):
        for i in range(len(path)-1):
            if self.is_pawn_move_ok(path[i], path[i+1], self.pawns[(player + 1) % 2]) == False:
                return False
        return True
    ...
    def play_action(self, action, player):
        ...
        """
        self.shortest_path_update[player] = False
        ...
        if kind == 'WH':
            ...
            # check whether this wall changes the shortest path
            if self.is_path_valid(self.shortest_path[player],  player) == False:
                self.shortest_path[player] = self.get_shortest_path(player)
            if self.is_path_valid(self.shortest_path[opp],  player) == False:
                self.shortest_path[opp] = self.get_shortest_path(opp)
            self.shortest_path_update =[True,  True]
        elif kind == 'WV':
            ...
            #print ("shortest path: action " + str(self.shortest_path[player])+ " for player" + str(player))
            #print ("Action " + str(x) + " " + str(y))
            if self.shortest_path[player] != []:
                 if (x,  y) == self.shortest_path[player][0]:
                    self.shortest_path[player] = self.shortest_path[player][1:]
                    #print("path shorter" + str(self.shortest_path[player] ) )
                    self.shortest_path_update[player] = True
    ...
    def get_score(self, player=PLAYER1):
      ...
      #  print ("play:" + str(player+1) + ' ' + str(self.shortest_path[0]) + "\nplay:  " + str(self.shortest_path[1]))
        return score
